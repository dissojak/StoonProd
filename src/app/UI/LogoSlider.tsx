...
type Logo = { src: string; invert?: boolean; wide?: boolean };
...
const shuffleAvoidConsecutiveDupes = (arr: Logo[]) => {
  const shuffled = [...arr];
  let attempt = 0;
  do {
        ...
        <div className="flex items-center space-x-10 animate-slide-xs sm:animate-slide-sm md:animate-slide-md">
          {fullList.map((logo, i) => (
            <Image
              ...
              width={80}
              height={32}
              className={`h-6 sm:h-8 ${logo.invert ? "invert dark:invert-0" : "dark:invert"} ${logo.wide ? "w-24 sm:w-32 object-contain" : ""}`}
